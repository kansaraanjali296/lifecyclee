...
def generate_file_name(url):
    ...
    # ----------------------------------------------url get file_name --------------------------------------------#
    file_name = url.split('://')[1].split("/")[0].replace(".", "_") + '_tender'
    return file_name
...
try:
    ...
    url = 'https://www.cenexahmedabad.nic.in/tender.html'
    driver.get(url)

    create_db_and_table_name = url.split('://')[1].split("/")[0].replace(".", "_") + '_tender_py'
    ...
    Competition = "NCB"
    Purchaser_Name = "cenexahmedabad"
    Pur_Add = "cenexahmedabad"
    Pur_URL = url
    ...
    for tr_data in tr_all_data:
        ...
        if not conn.execute(row_exists).fetchone():
            ...
            try:
                ...
                Documents_2 = ''
                logging.error('PDF File Not Available ' + str(Documents_2))
